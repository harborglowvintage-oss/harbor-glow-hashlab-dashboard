...
function initPSUFan() {
    ...
    function drawFan() {
        ...
        // Outer alien hull
        ctx.save();
        const gradient = ctx.createRadialGradient(centerX, centerY, 30, centerX, centerY, 65);
        ...
        for (let i = 0; i < 2; i++) {
            ctx.beginPath();
            ctx.arc(centerX, centerY, 65 - (i * 8), 0, Math.PI * 2);
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const x2 = centerX + Math.cos(angle) * 52;
            const y2 = centerY + Math.sin(angle) * 52;
